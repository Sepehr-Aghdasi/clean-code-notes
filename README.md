# Clean Code Notes

Personal notes from the ["Writing Clean Code"](https://www.udemy.com/course/writing-clean-code/) course on Udemy, with my own TypeScript examples.

My own notes and examples from the course.

## Contents

1. [Naming](./naming/naming.md)
2. [Comments & Formatting](./comments-formatting/comments-formatting.md)
3. [Functions & Methods](./functions-and-methods/functions-and-methods.md)
4. [Control Structures & Errors](./control-structures-and-errors/control-structures-and-errors.md)
